...
SELECT e.ename, e.empno, nvl(m.ename,'관리자없음') as 관리자이름
FROM emp e, emp m
where e.mgr = m.empno(+)
...
SELECT i.ename, i.hiredate
FROM emp e, emp i
where e.hiredate < i.hiredate and e.ename = 'WARD'
;
--42. SELF JOIN 을 사용하여 관리자보다 먼저 입사한 모든 사원의 이름 및 입사일을 관리자의 이름 및 입사일과 함께 출력하시오.
SELECT e.ename, e.hiredate, e.hiredate, m.ename, m.hiredate
FROM emp e, emp m
